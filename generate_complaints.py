"""Generate synthetic telecoms complaints using OpenAI GPT-5-mini."""
...
def _generate_batch(
    ...
    for attempt in range(1, MAX_RETRIES + 2):  # 1 initial + up to MAX_RETRIES
        ...
        print(f"    Attempt {attempt}: expected {expected}, got {len(complaints)}."
...
def generate_all(total: int = 5000, seed: int = 42) -> pd.DataFrame:
    ...
    for cell_idx, (urgency, emotion, _count) in enumerate(grid):
        ...
        for batch_idx, batch in enumerate(batches):
            print(f"  Batch {batch_idx + 1}/{len(batches)}: "
                  f"{len(batch)} complaints...")
            complaints = _generate_batch(client, system_prompt, batch)
...
def main() -> None:
    ...
    df = generate_all(total=args.total, seed=args.seed)
